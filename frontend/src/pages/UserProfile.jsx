...
const UserProfile = () => {
    ...
    useEffect(() => {
        const fetchUserRoom = async () => {
            try {
                const room = await api.get('active/');
                setUserRoom(room.data.results || []);
                console.log(room.data.results)
                console.log(profile)
            } catch (err) {
                console.error(err);
            }
    ...
    return (
        <div className="flex flex-col min-h-screen w-full bg-gray-50 ">
            ...
            <main className="flex-1 mt-24 px-4 animate-fadeIn">
                ...
                {userRoom.length > 0 ? (
                    <div className="container mx-auto max-w-6xl p-6 mb-5 bg-white rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-300 space-y-6 animate-fade-in">
                        {/* Header */}
                        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                            <h2 className="text-xl md:text-2xl font-semibold text-gray-800">Your House</h2>
                            <button 
                                className="px-5 py-2 bg-linear-to-r from-indigo-500 to-purple-500 text-white font-medium rounded-xl shadow-lg hover:shadow-2xl transition transform hover:-translate-y-1 hover:scale-105"
                        ...
                        <div className="w-full h-96 rounded-2xl overflow-hidden border border-gray-200 shadow-inner">
                            <RoomMap
                                ...
                                name={userRoom[0]?.room_name}
                            />
                        </div>
                    </div>
                ) : (
                    <div className="container mx-auto max-w-6xl p-6 mb-5 bg-white rounded-3xl shadow-xl animate-pulse space-y-6">
