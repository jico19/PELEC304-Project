...
class CustomUser(AbstractUser):
    ...
    occupation_status = models.CharField(
        max_length=20, choices=OCCUPATION_STATUS_CHOICES, default="Unemployed")
    ...
    marital_status = models.CharField(
        max_length=20, choices=MARITAL_STATUS_CHOICES, default="Single", blank=True)
    gender = models.CharField(
        max_length=20, choices=GENDER_CHOICES, default="Others", blank=True)
    permanent_address = models.CharField(
        max_length=300, default="", blank=True)
    role = models.CharField(
        max_length=20, choices=USER_ROLE_CHOICES, default="Tenant")
...
class Room(models.Model):
    ...
    room_availability = models.CharField(
        max_length=20, default="Available", choices=ROOM_AVAILABILITY_CHOICES)
...
class Reports(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=REPORT_STATUS_CHOICES, default="Unresolved")
...
class RentTransaction(models.Model):
    '''rent transaction '''
    transact_id = ShortUUIDField(
        length=5, max_length=30, prefix="tr_", alphabet="abcdefg1234", unique=True, editable=False)
    room = models.ForeignKey(Room, on_delete=models.CASCADE)
...
class ActiveRent(models.Model):
    ...
    rent_transaction = models.OneToOneField(
        RentTransaction, on_delete=models.CASCADE)
...
class Notification(models.Model):
    ...
    def __str__(self):
        return f"{self.notification_id} - {self.renter.username} - sender: {self.sender.username}"
...
''''
User uploads N files in frontend.

Create an application.

Loop uploaded files → create ApplicationDocument per file.
...
class LandlordApplication(models.Model):
    ...
    def __str__(self):
        return f"{self.applicant.username} - {self.status} - {self.applied_at}"